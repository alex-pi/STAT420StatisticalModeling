...
Since we are trying to prove/disprove a linear relationship between the predictor `(Bwt)` and the response `(Hwt)`, the null and alternative hypothesis for $\beta_1$ is:
...
get_coef_data <- function(coef) {
  ...
  #kable(table_data, 
  #    col.names = c("Estimate ","t value","p value"),
  #    caption = "Coefficients for Linear Model on MASS::cats dataset.")
}
...
With `90%` of confidence we say that, for a cat with `Body Weight` of `0 Kg` the mean of the `Heart Weight` is located in the interval: $\left(-1.502834, 0.7895096\right)$. Note that in real life the only `Heart Weight` value that makes sense would be 0, which in this case is in the `Confidence Interval`.
...
The interval for 2.1 Kg is wider in this case because 2.1 is farther from the mean of `x` (`Bwt`). We are less confident in areas with fewer data points (i.e. away from the mean), to account for that, the interval has to be wider.
...
#kable(pred_table, 
#      col.names = c("Body Weight (kg)","Predicted Hwt (g)","Lower Bound","Upper Bound"),
#      caption = "90% Prediction Intervals")
...
Since we are trying to prove/disprove a linear relationship between the predictor `(wind)` and the response `(ozone)`, the null and alternative hypothesis for $\beta_1$ is:
...
Since we are trying to prove/disprove a linear relationship between the predictor `(temp)` and the response `(ozone)`, the null and alternative hypothesis for $\beta_1$ is:
...
data_Q3b <- data.frame(
  ...
  "beta_0_hat" = c(beta_0, mean(beta_hats$beta_0_hats), 
                   sqrt(var_beta_0_hat), sd(beta_hats$beta_0_hats)),
  "beta_1_hat" = c(beta_1, mean(beta_hats$beta_1_hats), 
