...
ztest_graph <- function(ztests, color = "deepskyblue3") {
...
lrt_graph <- function(lrt_tests, dof, color = "deepskyblue3") {
  par(mfrow = c(1, 1), bg="ghostwhite")
...
*Checking `Wald Test` values*

```{r, fig.height = 4, fig.width = 7, fig.align = "center"}
ztest_graph(sim_resultf$wald_test, color = "gray")
...
*This shows evidence that the sample distribution for the `small sample` has changed and it might not be good enough for testing.*

*Checking `LRT Test` values*

```{r, fig.height = 4, fig.width = 7, fig.align = "center"}
lrt_graph(sim_resultf$lrt_test, dof, color = "gray")
...
*We can go a step forward and take a look at the `statistical decisions` with $\alpha = 0.05$.*
...
From our true model we know that $\beta_2=0$ and $\beta_3=0$, so from the `LRT` test we expect to `Reject the Null Hypothesis`. In the table above we see that using $\alpha=0.05$, we have a larger proportion of `Rejections of the Null Hypothesis` for the `large sample` compared to what we get with the `small sample`. In this case the `small sample` might not be large enough to use a $\chi^2$ distribution successfully.
...
Relative to `wisc_add_3` (`radius + smoothness + texture`), `wisc_add_all` and `wisc_aic` are **overfitting**.
...
In this case we **do not want** many `False Negative` errors. In other words, we want to **minimize** the chances of missing actual `Malignant Tumors`. This will increase the `False Positive` errors, which is less costly in this context.

Thus, we are looking for a cutoff with high Sensitivity. We can easily discard cutoffs after *~0.8* as `Sensitivity` drops faster after that point.

For this particular case, a reasonable cutoff could be between **0.1** and **0.3**. But that is a subjective decision, expert advice in the field will help to make this decision.
