---
title: "Week 8 - Homework - STAT 420, Summer 2021"
...
**More details on the cubic model**

Here is a graph of how the `cubic` model fits the data.
...
The `normality assumption` doesn't seem to be violated. The `p-value` is **r diags_d3$p_val`**. *We must considered that this data set is very small, the tests might not be very accurate.*

Cook's distances for the `cubic model` are:

```{r}
diags_d3$cookd$values
```

No influential points were found with the heuristic:
