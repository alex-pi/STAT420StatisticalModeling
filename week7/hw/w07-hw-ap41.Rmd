---
title: "Week 7 - Homework - STAT 420, Summer 2021"
...
By looking at the plot, it is not very clear if the data points for different `type` of vehicles are either being underestimated or overestimated.
...
This model with the factor variable `type` seems to fit the model somewhat better than the previous SLR model, that said, it did not improve much. $R^2 = `r r_2`$
...
Now we have a different `slope` for each `type` of vehicle which seems to fit a bit better each of the 3 clusters of data points. That said, the slopes are not that different so the improvement might not be very significant, we would need to investigate a bit more.
...
The term $(\beta_1 + \gamma_2)$ is of course the `slope_car` we calculated above. So the increase is **`r slope_car`**.
...
The additive model we are fitting is:
...
where:
- $x$ is the Age
- $v_e$ is the factor variable for Education
- $v_i$ is the factor variable for Income
- $v_s$ is the factor variable for Sex
...
Now we fit a model that includes 2 interactions:
...
$$
Here we are interested in this term: $(\beta_a + \beta_{ai} + \beta_{as} + \beta_{ais})$

We can calculate it using the `model`'s coefficients.
...
Therefore, the estimate of the change in average `Blood` for a one-unit increase in `Age` for a highly educated, low income, male patient is **`r mean_blood_change`**
...
We prefer the model with interactions `days_int` as at least one of the added predictors (interactions) is significant.
...
For completeness, below we calculate the estimated mean change in length of hospital stay.
...
The additive model defined as follows:
...
The fact that both tests generate the same `p-values` indicates that testing for these `hypothesis`:

$$H_0: \beta_1 = 0$$
$$H_0: \mu_A = \mu_B$$

will give the same results for any given $\alpha$
...
The difference might be explained by the reference level the `lm` function uses (in this case `A`) compared to what the `t.test` function selects.