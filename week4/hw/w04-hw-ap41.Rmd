---
title: "Week 4 - Homework - STAT 420, Summer 2021"
...
The following table shows the corresponding `t values` and `p values` for each of the fitted coefficients:

```{r}
summary(nutrition_model)$coef
gen_kable(summary(nutrition_model)$coef, caption = "Estimated Coefficients")
```

(Note: Some of the `p values` are so small that are shown as `0`)
...
- $\hat\beta_0$, `(Intercept)` Represents the number of calories of a food with `0` Fat, Sugar and Sodium. Of course this is an unrealistic case in the model.
...
Note that we say `extra` to express that `Calories` increase when the predictors also increase. This is explained by the positive sign of the Estimates $\hat\beta_j$.
...
*Prediction:* **`r fr(cals, digits = 4)` Cal**.
...
left <- mean(nutrition$Calories) - s_y
right <- mean(nutrition$Calories) + s_y
```

$s_y$ explains how spread (far from the mean in average) the response observations (`Calories`) are, in this case, ~68% are in the range: **(`r fr(left)`,`r fr(right)`)**

The `unbiased` estimated $s_e$ from our model, represents how far our estimates are from the actual observed response (`Calories`). We expect this to be small.
...
With `95%` confidence we can say that the mean `Calories` for a food with `0` value in the predictors is in the range: $\left(`r fr(conf_beta_0[1])`, `r fr(conf_beta_0[2])`\right)$. Note that all values in that range are unrealistic for that particular case, but we can ignore this extrapolation.
...
For a food with `Fat=15, Sugar=0g and Sodium 260mg`, we are 99% confident that the mean of the response (`Calories`) is in the range: $\left(`r fr(conf_new[2])`, `r fr(conf_new[3])`\right)$
...
The prediction interval considering the estimates variability (i.e. $\sigma^2$) is $\left(`r fr(pred_inter[2])`, `r fr(pred_inter[3])`\right)$. We expect with 99% confidence that the estimate for the `Crunchy Taco Supreme` will be in that range.
...
- The value of the test statistic: **`r fr(anova_12[2, "F"])`**
- The p-value of the test: **`r fr(anova_12[2, "Pr(>F)"])`**
...
r2_rss_data <- data.frame(
  ...
  "Predictors" = c("GA + SV", "GA + SV + SA + MIN + SO"),
...
gen_kable(r2_rss_data, caption = "Model 1 vs Model 2")
model_a <- model_1
```
...
* Pros
  ...
  * The `Residual Sum of squares` seems significantly smaller than it is for `model 1`

* Cons
  * `model 2` has more predictors, making it harder in interpret.
  * One of those extra predictors `SO` seems to be not very significant with a relatively high `p-value` of `r summary(model_2)$coef["SO", "Pr(>|t|)"]`, at least when combined with the other 4 predictors.
 
I would make a case for picking `model 1`:

- It is smaller and easier to interpret.
- Its $R^2$ is very close to the one for `model 2`.
- It is a good base to explore if `SA` and `MIN` are worth keeping in the model.
...
$$H_0: \beta_{SA} = \beta_{MIN} = \beta_{SO} = \beta_{FISRT} = \beta_{LAST} = \beta_{Active} = \beta_{GP} = \beta_{PIM} = 0$$

```{r}
(anova_a3 <- anova(model_a, model_3))
```

- The value of the test statistic: **`r fr(anova_a3[2, "F"])`**
- The p-value of the test: **`r fr(anova_a3[2, "Pr(>F)"])`**
...
At least one of the predictors in the null hypothesis is significant given that `GA, SV` are already in the model.

Let's take a look to the $R^2$ and the `Residual Sum of squares`

```{r}
r2_rss_data <- data.frame(
  row.names = c("Model 1", "Model 3"),
  "Predictors" = c("GA + SV", "(All Predictors)"),
...
gen_kable(r2_rss_data, caption = "Model 1 vs Model 3")
model_b <- model_a
```

Based only in the test for the `F statistic` we would prefer `model 3` but:

- For both models (1 and 3) the $R^2$ is very close.
- `model 1` is smaller and easier to interpret than `model 3`.
- Some of the predictors in `model 3` have a very low `p value`.

I would still prefer `model 1`.
...
- The value of the test statistic: **`r fr(t_val)`**
- The p-value of the test: **`r fr(p_val)`**
...
`SV` is significant given that `GA` is already in the model.
...
$sigma$ reported by `R` = **`r summary(ozone_model)$sigma`**
...
Calculated $R^2 = `r r_2`$

$R^2$ reported by `R` = **`r summary(ozone_model)$r.squared`**
...
model_woy <- lm(mpg ~ weight + origin + year, data = auto_trn)
...
rmse_trn_woy <- rmse(model_woy, auto_trn$mpg, data = auto_trn)
...
rmse_tst_woy <- rmse(model_woy, auto_tst$mpg, data = auto_tst)

rmse_summary <- data.frame(
  "Predictors" = c("(All)", "displacement", "weight + year", "weight + acceleration + year", "weight + origin + year"),
  "Train RMSE" = c(rmse_trn_full, rmse_trn_d, rmse_trn_wy, rmse_trn_way, rmse_trn_woy),
  "Test RMSE" = c(rmse_tst_full, rmse_tst_d, rmse_tst_wy, rmse_tst_way, rmse_tst_woy)
)

gen_kable(rmse_summary, col_names = c("Predictors","Train RMSE", "Test RMSE"), add_row_names = FALSE)

```

`model_woy` with predictors `weight + origin + year` has the lowest `RMSE`. Although the `full` model has a similar `RMSE`, **`model_woy` is a better choice** since it is smaller and easier to interpret.
...
The `mean` and `variance` values obtained from simulations are close to the expected ones. Given that, the `histogram` for the estimated $\hat\beta_1$ and `curve` for the true distribution match.
...
```{r}
hist(beta_5_pval, breaks = 20,
     xlab = expression("p values for " ~ H[0]: hat(beta)[5] ~ "= 0"), main = "p values Distribution", border = "dodgerblue")
```
